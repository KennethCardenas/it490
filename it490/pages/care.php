...
function deleteCareLogFromDatabase($conn, $logId, $userId) {
...
if (isset($_GET['delete'])) {
    ...
    try {
        $payload = [
            'type' => 'delete_care_log',
            'log_id' => $logId,
            'user_id' => $user['id'] ?? 0
        ];

        $deleteResp = @sendMessage($payload);

        if (empty($deleteResp) || ($deleteResp['status'] ?? '') !== 'success') {
            if (deleteCareLogFromDatabase($conn, $logId, $user['id'] ?? 0)) {
                $redirectMsg = urlencode('Care log deleted successfully');
            } else {
                throw new Exception('Failed to delete care log');
            }
        } else {
            $redirectMsg = urlencode($deleteResp['message'] ?? 'Care log deleted successfully');
        }
                            ...
                            <tbody>
                                <?php foreach ($logs as $log): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#" class="delete-care-log" data-log-id="<?= $log['id'] ?>" title="Delete">
...
<div id="deleteModal" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <p>Are you sure you want to delete this care log?</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const deleteLinks = document.querySelectorAll('.delete-care-log');
    ...
    deleteLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            confirmDeleteBtn.href = `care.php?dog_id=<?= $dogId ?>&delete=${logId}`;
